...
$('.show-call').on('click', function () {
    $('.modal-callback').find('input[name="about-form"]').val($(this).data('about-form'));
    ...
    click: function (e) {
        ...
        allStars.toArray().forEach(function (x, number) {
            allStars.eq(number).find('img').toArray().forEach(function (value, index) {
                if (starCount[number] > index) allStars.eq(number).find('img').eq(index).prop('src', 'img/train-offer/star-yellow.svg');
                else allStars.eq(number).find('img').eq(index).prop('src', 'img/train-offer/star-grey.svg');
            });
        });
    }
    , mouseenter: function () {
